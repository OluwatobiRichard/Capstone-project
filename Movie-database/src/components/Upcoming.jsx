  ...
  useEffect(() => {
    const fetchMovies = async () => {
      try {
        ...
        console.log("Upcoming movies data:", data); // For debugging
  ...
  return (
    <div className="h-[32.625rem] w-screen bg-[#1B1B1B] overflow-hidden items-center justify-center py-10 mx-auto space-y-8">
      <div className="flex flex-col text-white m-auto">
        <h1 className="text-center text-[1.375rem]">Upcoming Movies</h1>
      </div>
      <div className="flex flex-row gap-3 justify-center items-center text-[0.75rem] overflow-hidden px-4">
        {movies.map((movie) => (
          <div
            key={movie.id}
